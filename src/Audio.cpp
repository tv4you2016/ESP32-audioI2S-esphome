...
bool Audio::connecttohost(const char *host, const char *user, const char *pwd) {
  ...
  //        m_timeout_ms = UINT16_MAX;  // [WiFiClient.cpp:253] connect(): select returned due to timeout 250 ms for fd
  //        48
  //    } fix in V2.0.8
...
void Audio::computeLimit() {   // is calculated when the volume or balance changes
  ...
  switch (m_curve) {
    ...
    case 1:  // logarithmic
      ...
      if (m_vol > 0) {
        v = m_vol *
            ((std::exp(log1 + (m_vol - 1) * (std::log(m_vol_steps) - log1) / (m_vol_steps - 1))) / m_vol_steps) /
            m_vol_steps;
      } else {
        v = 0;
      }
